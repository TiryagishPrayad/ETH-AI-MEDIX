...
function getAccessAddresses() external view returns (address[] memory) {
    Access[] storage accesses = accessList[msg.sender];
    address[] memory addresses = new address[](accesses.length);

    for (uint256 i = 0; i < accesses.length; i++) {
        addresses[i] = accesses[i].user;
    }
...
receive() external payable {
    ...
    function sendTransaction(address payable recipient) external payable {
        require(msg.value > 0, "Must send non-zero Ether value");
